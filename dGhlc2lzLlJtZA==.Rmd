...
```{r}
run_model_activations <- function(trials,probe){
  memory <- trials
...
#probe memory with cue only
  ...
  # get activations
  activations <- sum(similarities^15)
...
for(i in 1:4){
  ...
  freq_A <- run_model_activations(trials=trials,probe = rep(c(1,1,1),each=10))
  freq_B <- run_model_activations(trials=trials,probe = rep(c(1,0,1),each=10))
  freq_C <- run_model_activations(trials=trials,probe = rep(c(0,1,1),each=10))
  freq_D <- run_model_activations(trials=trials,probe = rep(c(0,0,1),each=10))
...
```{r, fig.cap = "Mean model contingency ratings derived from model estimates of frequency judgments.", fig.width=5, fig.height = 3, fig.align='center'}
# plot
